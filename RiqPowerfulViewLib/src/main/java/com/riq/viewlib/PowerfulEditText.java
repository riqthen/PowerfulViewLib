...
/**
 * 可以给EditText自动添加删除全部文本的按钮   app:funcType="canClear"
 * 设置密码显示与否   app:funcType="canWatchPwd"
 * 在输入框两端设置图片,且可以控制图片大小   drawableLeft drawableRight leftDrawableHeight leftDrawableWidth...
 * 给右边按钮设置点击事件   setOnRightClickListener
 */

/**
 * 不能再设置了funcType的同时设置drawableRight
 * 设置了funcType的同时不能设置右边点击事件
 */
public class PowerfulEditText extends EditText {
    ...
    public PowerfulEditText(Context context, AttributeSet attrs, int defStyle) {
        ...
        eyeCloseResourceId = ta.getResourceId(R.styleable.PowerfulEditText_eyeClose, R.mipmap.eye_close);
        eyeOpenResourceId = ta.getResourceId(R.styleable.PowerfulEditText_eyeOpen, R.mipmap.eye_open);
